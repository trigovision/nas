...
from nas.utils import get_same_padding, min_divisible_value, SEModule, ShuffleLayer
from nas.utils import MyNetwork, MyModule
from nas.utils import build_activation, make_divisible
...
class IdentityLayer(My2DLayer):
    def __init__(
        self, in_channels, out_channels, use_bn=False, act_func=None, dropout_rate=0, ops_order="weight_bn_act"
    ):
    ...
    def __init__(
        ...
        depth_conv_modules = [
            (
                "conv",
                nn.Conv2d(
                    feature_dim, feature_dim, kernel_size, stride, pad, dilation=dilation, groups=groups, bias=False
                ),
            ),
            ("bn", nn.BatchNorm2d(feature_dim)),
            ("act", build_activation(self.act_func, inplace=True)),
