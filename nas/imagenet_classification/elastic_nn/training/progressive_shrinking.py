...
from nas.utils import AverageMeter, cross_entropy_loss_with_soft_target
...
from nas.imagenet_classification.run_manager import DistributedRunManager
...
def train_elastic_depth(train_func, run_manager, args, validate_func_dict):
    ...
    if run_manager.start_epoch == 0 and not args.resume:
        ...
        run_manager.write_log(
            "%.3f\t%.3f\t%.3f\t%s" % validate(run_manager, is_test=True, **validate_func_dict),
            "valid",
        )
    else:
        assert args.resume
...
def train_elastic_width_mult(train_func, run_manager, args, validate_func_dict):
    ...
    if run_manager.start_epoch == 0 and not args.resume:
        ...
        if current_stage == 0:
            ...
            run_manager.write_log(
                "reorganize_middle_weights (expand_ratio_stage=%d)" % (len(dynamic_net.expand_ratio_list) - 1),
                "valid",
            )
            try:
