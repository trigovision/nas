def load_state(net, checkpoint, arch_name: str):
    if arch_name == "mbv3":
        load_state_mbv3(net, checkpoint)
    elif arch_name == "resnet":
        load_state_resnet(net, checkpoint)
    else:
        raise NotImplementedError()


def load_state_mbv3(net, checkpoint):
...
def load_state_resnet(net, checkpoint):
    source_state = checkpoint["state_dict"]
    target_state = net.state_dict()
    new_target_state = target_state.copy()
